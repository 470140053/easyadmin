...
use App\Http\Admin\Assemble\QiniuConfigAssemble;

class QiniuConfigModel extends BaseModel
{

    //对应的表名
    protected $tableName = MYSQL_PREFIX . 'qiniu_config';

    //主键字段
    public $primaryKey = 'id';

    protected $assemble = QiniuConfigAssemble::class;
    ...
    public function schemaInfo(bool $isCache = true): Table
    {
        ...
        $table->colVarChar('access_key', 255);
        $table->colVarChar('secret_key', 255);
        $table->colVarChar('storage', 32);
        $table->colVarChar('host_url', 255);
        $table->colVarChar('com_storage', 32);
        $table->colVarChar('com_host_url', 255);
