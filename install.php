...
// 安装包目录
define('INSTALL_PATH', APP_PATH . 'Http' .  DS . 'Install' . DS);
...
$dbPrefix = '`tp_';
//数据库配置文件
$dbConfigFile = './dev.php';


// 锁定的文件
$lockFile = INSTALL_PATH . 'install.lock';
if (is_file($lockFile)) {
    $errInfo = "当前已经安装{$sitename}，如果需要重新安装，请手动移除App/Http/Install/install.lock文件";
...
if (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $mysqlHostport = isset($_POST['mysqlPort']) ? $_POST['mysqlPort'] : 3306;
    ...
    $mysqlPrefix = isset($_POST['mysqlPrefix']) ? $_POST['mysqlPrefix'] : 'tp_';
    ...
    try {
        //检测能否读取安装文件
        $sql = @file_get_contents(INSTALL_PATH . $sitename .'.sql');
        if (!$sql) {
            throw new Exception("无法读取App/Http/Install/{$sitename}.sql文件，请检查是否有读权限");
        }
        $sql = str_replace($dbPrefix, "`{$mysqlPrefix}", $sql);
        ...
        $config = preg_replace_callback("/'(host|database|user|password|port|prefix)'(\s+)=>(\s+)(.*)\,/", $callback, $config);
        
        //检测能否成功写入数据库配置
        $result = @file_put_contents($dbConfigFile, $config);
        if (!$result) {
            throw new Exception("无法写入数据库信息到/.dev.php文件，请检查是否有写权限");
        }

        //检测能否成功写入lock文件
        $result = @file_put_contents($lockFile, 1);
        if (!$result) {
            throw new Exception("无法写入安装锁定到App/Http/Install/install.lock文件，请检查是否有写权限");
        }

        $newSalt = substr(md5(uniqid(true)), 0, 6);
        $newPassword = md5(md5($adminPassword . 'tp_auth_user') . $newSalt);
        
        $pdo->query("UPDATE {$mysqlPrefix}auth_user SET username = '{$adminUsername}', password = '{$newPassword}', salt = '{$newSalt}' WHERE username = 'admin'");
        echo "success";
        ...
        <form method="post">
            ...
            <div class="form-group">
                ...
                <div class="form-field">
                    ...
                    <input type="text" name="mysqlPort" value="8889" required="">
                </div>
                ...
                <div class="form-field">
                    ...
                    <input type="text" name="mysqlPrefix" value="tp_">
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="form-field">
                    <label>重复密码</label>
                    <input type="password" name="adminPasswordConfirmation" value="" required="">
                </div>
            </div>
