...
abstract class Addons
{
    ...
    public function __construct()
    {
        $name = $this->getName();
        // 获取当前插件目录
        $this->addons_path = EASYSWOOLE_ROOT.ADDON_PATH . $name . DIRECTORY_SEPARATOR;
    }
    ...
    final public function getInfo($name = '')
    {
        ...
        $redis = eRedis();
        $result = $redis->hGetAll($this->infoRange);
        if(!empty($result[$name])) {
            return json_decode($result[$name],true);
        }
        
        $info_file = $this->addons_path . 'info.ini';
        if (is_file($info_file)) {
            $info = parse($info_file);
            $redis->hSet($this->infoRange,$name,json_encode($info));
        }
        return $info;
    }
